...
import { GitHubLinkModal } from '../components/mvp/GitHubLinkModal'; // Import GitHubLinkModal
...
export const EditMVPPage: React.FC = () => {
  ...
  // State for GitHubLinkModal
  const [gitHubModalOpen, setGitHubModalOpen] = useState(false);
  const [currentMvpData, setCurrentMvpData] = useState<MVP | null>(null); // To pass to GitHubLinkModal

  const fetchMVP = async () => {
    setLoadingMVP(true);
    setMvpLoadError(null);
    try {
      const mvp = await APIService.getMVPById(mvpId!); // mvpId is guaranteed to exist here
      if (mvp) {
        ...
        setCurrentMvpData(mvp); // Store the fetched MVP data
        setFormData({
          title: mvp.title,
          tagline: mvp.tagline,
          ...
          access_tier: mvp.access_tier || 'free', // Pre-fill access_tier or default to 'free'
          price: mvp.price || undefined, // Pre-fill price or undefined
        });
  ...
  useEffect(() => {
    ...
    if (!user.is_seller_approved && user.role !== 'admin' && user.role !== 'both') {
      navigate('/dashboard');
      return;
    }
  ...
  const handleCloseGitHubModal = () => {
    ...
    setSelectedMVP(null); // Clear selected MVP when modal closes
  };

  const handleGitHubSuccess = () => {
    handleCloseGitHubModal();
    fetchMVP(); // Re-fetch MVP data to update GitHub status
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const updates: Partial<MVP> = {
        ...
        access_tier: formData.access_tier, // Include access_tier in updates
        // Conditionally include price only if access_tier is 'one_time_sale'
        price: formData.access_tier === 'one_time_sale' ? formData.price : null,
      };
                ...
                {currentMvpData && (
                  <div className="mt-6">
                    ...
                    >
                      ...
                      <span>Manage GitHub Integration</span>
                    </GlossyButton>
                  </div>
                )}
              </div>
